     ...
     {/* <Box sx={{ 
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} sm={6} lg={2}>
          <SummaryCard
            title="Success Rate"
            value={`${summary.totalRequested ? Math.round((summary.totalApproved / summary.totalRequested) * 100) : 0}%`}
        ...
        <Grid item xs={12} sm={6} lg={2}>
          <SummaryCard
            title="Avg Amount"
            value={formatCurrency(summary.totalRequested ? summary.totalAmount / summary.totalRequested : 0)}
            ...
            <Table>
              <TableHead sx={{ backgroundColor: '#f8f9fa' }}>
                <TableRow>
                  ...
                  {currentTab === 1 && <TableCell sx={{ fontWeight: 600 }}>Approved By</TableCell>}
                  <TableCell sx={{ fontWeight: 600 }}>Created At</TableCell>
                  {currentTab === 0 && <TableCell sx={{ fontWeight: 600 }}>Actions</TableCell>}
                </TableRow>
              </TableHead>
              <TableBody>
                {filteredData.map((transaction) => (
                  <TableRow key={transaction._id} hover sx={{ '&:hover': { backgroundColor: '#f8f9fa' } }}>
                    <TableCell>
                    ...
                    <TableCell>
                      <Chip
                        icon={transaction.approved ? <CheckCircleIcon /> : <PendingIcon />}
                    ...
                    <TableCell>
                      <Typography variant="body2" fontFamily="monospace" sx={{
                        ...
                        <a href={`https://transactioninfo.ethiotelecom.et/receipt/${transaction.reference?.toString().toUpperCase()}`} target="_blank">{transaction.reference}</a>
                      </Typography>
                    </TableCell>
                    {currentTab === 1 && (
                      <TableCell>
                ...
                {filteredData.length === 0 && !loading && (
                  <TableRow>
                    <TableCell colSpan={currentTab === 0 ? 9 : 8} align="center" sx={{ py: 8 }}>
                      <Box sx={{ textAlign: 'center' }}>
                        <Typography variant="h6" color="text.secondary" gutterBottom>
                          {currentTab === 0 ? 'No pending requests' : 'No approved requests'}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                          {currentTab === 0 
                  ...
                  <Stack spacing={2}>
                    <Box>
                      ...
                      <Typography variant="body1" fontFamily="monospace" sx={{ fontWeight: 600 }}>
                        {approvalModal.transaction.trxid || approvalModal.transaction._id.slice(-8)}
                      </Typography>
                    </Box>
                    <Box>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      ...
                      <Box>
                        ...
                        <TextField
                            ...
                            onChange={e => setTelebirrReferenceData(e.target.value)}
              ...
              {approvalModal.transaction.note && (
                <Grid item xs={12}>
                  <Paper sx={{ p: 3, backgroundColor: '#fff3cd', borderRadius: 2, border: '1px solid #ffeaa7' }}>
                    <Typography variant="h6" gutterBottom sx={{ fontWeight: 600, color: '#856404' }}>
                      Notes
                    </Typography>
        ...
        <DialogContent sx={{ p: 4 }}>
          <Alert severity="warning" sx={{ mb: 3, borderRadius: 2 }}>
            ...
            <Typography variant="body2" sx={{ mt: 1 }}>
              This will permanently mark the request as void and cannot be reversed.
            </Typography>
          </Alert>
            ...
            <Paper sx={{ p: 3, backgroundColor: '#f8f9fa', borderRadius: 2 }}>
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <Typography variant="caption" color="text.secondary">Transaction ID</Typography>
                  <Typography variant="body1" fontFamily="monospace" sx={{ fontWeight: 600 }}>
                    {voidModal.transaction.trxid || voidModal.transaction._id.slice(-8)}
                  </Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="caption" color="text.secondary">Amount</Typography>
